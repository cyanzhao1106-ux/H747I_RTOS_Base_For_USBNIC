...
USBH_StatusTypeDef USBH_LL_Init(USBH_HandleTypeDef *phost)
{
  ...
  /* OTG internal DMA enabled: the DMA completion path tracks the OUT data
     toggle per packet-count parity (correct for multi-packet bulk OUT frames),
     unlike the slave/FIFO path which mis-toggles even-packet transfers and
     corrupts large TX frames (e.g. ICMP replies for ping -s 600). With DMA on,
     every buffer handed to a USB transfer MUST be 4-byte aligned (see the
     RTKNIC buffers and the OCP bounce buffer). D-cache is not enabled on this
     core, so no cache maintenance is required. */
