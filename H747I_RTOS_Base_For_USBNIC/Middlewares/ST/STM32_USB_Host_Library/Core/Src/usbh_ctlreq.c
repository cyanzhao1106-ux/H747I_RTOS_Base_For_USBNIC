...
/**
  * @brief  USBH_Get_CfgDesc_Idx
  *         Issues Configuration Descriptor request for a specific configuration
  *         index (additive helper for multi-configuration devices, e.g. Realtek
  *         USB NIC chips that expose vendor + CDC-ECM/NCM configurations). On
  *         success the descriptor is parsed into phost->device.CfgDesc so that
  *         USBH_FindInterface() and the endpoint tables reflect the requested
  *         configuration.
  * @param  phost: Host Handle
  * @param  cfg_idx: Zero-based configuration index (0..bNumConfigurations-1)
  * @param  length: Length of the descriptor
  * @retval USBH Status
  */
USBH_StatusTypeDef USBH_Get_CfgDesc_Idx(USBH_HandleTypeDef *phost,
                                        uint8_t cfg_idx, uint16_t length)
{
  /* For the compatibility for 8152B, use seperate method to get the config
     descriptor: first step get 9Bytes for length, 2nd step for the real desc
     length to avoid the EP0 NAK issue.

     This two-stage fetch mirrors Windows and the standard ST enumeration flow
     (ENUM_GET_CFG_DESC -> ENUM_GET_FULL_CFG_DESC): first read the 9-byte
     configuration header to learn wTotalLength, then read exactly that many
     bytes. Some devices (notably the Realtek RTL8152B's non-default CDC-ECM
     configuration at index 1) NAK the EP0 data stage forever when asked for
     more bytes than the descriptor actually contains, wedging the control
     state machine permanently. Requesting the exact length avoids that and is
     what the Windows USB stack does. This function is non-blocking and called
     repeatedly until it returns a non-BUSY status. */
  static uint8_t  stage = 0U;      /* 0 = read 9-byte header, 1 = read full body */
  static uint16_t total_len = 0U;  /* wTotalLength captured from the header       */
  ...
  /* A change of requested index always restarts at the header stage; this also
     clears any stale stage left behind if a previous fetch was abandoned
     mid-transfer (e.g. on device disconnect/re-enumeration). */
  if (cfg_idx != last_idx)
  {
    stage = 0U;
    last_idx = cfg_idx;
  ...
  if (stage == 0U)
  {
    /* Stage 1: fetch the 9-byte configuration descriptor header only */
    status = USBH_GetDescriptor(phost, (USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD),
                                ((uint16_t)USB_DESC_CONFIGURATION | (uint16_t)cfg_idx),
                                pData, USB_CONFIGURATION_DESC_SIZE);

    if (status == USBH_OK)
    {
      /* wTotalLength is little-endian at offset 2..3 of the config descriptor */
      total_len = (uint16_t)((uint16_t)pData[2] | ((uint16_t)pData[3] << 8));

      if (total_len < (uint16_t)USB_CONFIGURATION_DESC_SIZE)
      {
        total_len = (uint16_t)USB_CONFIGURATION_DESC_SIZE;
      }
      if (total_len > length)
      {
        total_len = length;
      ...
      stage = 1U;
      status = USBH_BUSY;   /* proceed to the full read on the next invocation */
    }
    /* USBH_BUSY: stay in stage 0; USBH_FAIL/NOT_SUPPORTED: reported as-is */
  }
  else
  {
    /* Stage 2: fetch the full configuration descriptor at its exact length */
    status = USBH_GetDescriptor(phost, (USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD),
                                ((uint16_t)USB_DESC_CONFIGURATION | (uint16_t)cfg_idx),
                                pData, total_len);
    ...
    {
      /* Commands successfully sent and Response Received */
      status = USBH_ParseCfgDesc(phost, pData, total_len);
      stage = 0U;
    }
    else if ((status == USBH_FAIL) || (status == USBH_NOT_SUPPORTED))
    {
      stage = 0U;   /* reset so a retry starts cleanly from the header */
    }
    else
    {
      /* USBH_BUSY: remain in stage 1 */
    }
  }
